...
import MockStateArtifact from "../../artifacts/contracts/mock/MockState.sol/MockState.json";
import { MockState } from "../../typechain/MockState";
...
describe("State", function () {
  ...
  beforeEach(async function () {
    setters = (await deployContract(owner, MockStateArtifact, [])) as MockState;
  });
  ...
  describe("erc20 details", function () {
    describe("name", function () {
      it("increments total bonded", async function () {
        expect(await setters.name()).to.be.equal("Real Stable Dollar Stake");
      });
    });

    describe("symbol", function () {
      it("increments total bonded", async function () {
        expect(await setters.symbol()).to.be.equal("RSDS");
      });
    });
